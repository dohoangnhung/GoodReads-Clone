...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        // if user is not authenticated, check user again database
        if (userEmail != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            UserDetails userDetails = this.userDetailsService.loadUserByUsername(userEmail);

            // allow the process only when the token does not expire and revoke
            boolean isTokenValid = tokenRepository.findByToken(jwt)
                    .map(token -> !token.isExpired() && !token.isRevoked())
                    .orElse(false);

            // check whether the token valid or not
            // if valid, update the SecurityContextHolder and send DispatcherServlet
            if (jwtService.isTokenValid(jwt, userDetails) && isTokenValid) {
                // to update the SecurityContextHolder
